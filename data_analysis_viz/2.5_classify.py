...
df_temp = df[['Rating','Rating_z_score','kmeans_4_cluster','kmeans_5_cluster','5_rando_classes','4_rando_classes','participant_gender_id','participant_sexual_orientation','participant_voice_id','participant_cis_trans',
...
df = df.drop(['Rating','Rating_z_score','kmeans_4_cluster','kmeans_5_cluster','5_rando_classes','4_rando_classes','participant_gender_id','participant_sexual_orientation','participant_voice_id','participant_cis_trans',
                'participant_prox_social','participant_prox_affiliation', 'participant_prox_media', 'participant_race','participant_race_hispanic','eng_primary_early','eng_primary_current',
                'participant_other_langs','participant_race_free_response','participant_gender_pso_free_response', 'participant_age', 'deaf_hoh', 'Participant',
                'survey_experience','survey_feedback','Condition','WAV','color_3_cluster','color_4_cluster','color_5_cluster','spectral_S_start','spectral_Z_start','spectral_F_start','spectral_V_start','spectral_JH_start','spectral_SH_start', 'spectral_TH_start', 'spectral_DH_start',
...
for type in type_list:
    if type == 'baseline':
        ...
        df_imp = df_imp.drop('kmeans_3_cluster', axis=1)

        # make cluster dfs with imputed grand means
        df_cluster_dummies = pd.get_dummies(df_imp['3_rando_classes'], prefix='cluster')
        df_grand = pd.concat([df_imp, df_cluster_dummies], axis=1)
        df_sans = df_grand.drop('3_rando_classes', axis=1)
        ...
        labels = np.array(df_imp['3_rando_classes'])
        # Remove the labels from the features
        # axis 1 refers to the columns
        df_imp = df_imp.drop('3_rando_classes', axis = 1)
        ...
        cluster_dict  = {'df_group_0':df_group_0,'df_group_1':df_group_1,'df_group_2':df_group_2}
    ...
    if type == 'raw_ablation':
        ...
        for i in range(len(df_orig.columns)-2):
            ###### GRAND ####### Big ol' df with imputed means on everyone
            # Labels are the values we want to predict
            labels = np.array(df_imp['kmeans_3_cluster'])
            # Remove the labels from the features
            # axis 1 refers to the columns
            df_array = df_imp.drop('kmeans_3_cluster', axis = 1)
    ...
    if type == 'PCA_red_corr':
        ...
        y = data.loc[:,['kmeans_3_cluster']].values
        ...
        finalDf = pd.concat([principalDf, data['kmeans_3_cluster']], axis=1)
        ...
        for i in range(0,10):
            ...
            # Labels are the values we want to predict
            labels = np.array(finalDf['kmeans_3_cluster'])
            # Remove the labels from the features
            # axis 1 refers to the columns
            finalDf_array = finalDf.drop('kmeans_3_cluster', axis = 1)
